#---- CUTOFF SELECTION ----#
set.label = 'strict'
results.path = 'overlap_results/strict'
...
remove_conv_clade_pairs = list(c('Pocilloporid', 'Galaxia'))
remove_conv_clade_spp_pairs = list(c('Pocilloporid', 'Pastreoide'), c('Galaxia', 'Pastreoide'))
...
bs.min.lineage = 2             #number of target lineages that must be significant to keep gene
...
min.convergent.changes = 3                   #the number of lineages that must show a convergence event at a site for it to be kept. Note these don't all have to be the SAME convergent change. For example two clades could show P>V at a position, and two others could show P>I. This would return 4 lineages showing convergence.


#branch-sites flagged sites filters
bs.site.post.cutoff = 0.9        #cutoff for posterior probability the site has a dN/dS > 1
...
min.flagged.tests = 2            #the site must have been flagged in >= this number of tests within a phenotype for the site to pass
