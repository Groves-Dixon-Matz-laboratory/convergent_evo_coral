#branch_sites_overlapV5.R
#this script does the same things as branch_sites_overlap.R, but tidied up
...
# source("scripts_for_analysis/relaxed_cutoff_set.R") #RELAXED CUTOFFS
source("scripts_for_analysis/moderate_cutoff_set.R")  #MODERATE CUTOFFS
figPref =paste(results.path, 'figures', sep='/')
...
conv_filt <- conv_dat %>%
	...
	select('ortholog', 'pos', 'site', 'b1post', 'b2post', 'b1Clade', 'b2Clade', 'conv_pair', 'type', 'genConvergent', 'conv_pass', 'conv_ttPass', 'conv_taPass', 'conv_aaPass') %>%
	as_tibble
	
#prove to myself I only need the single conv_pass factor
#how many positions pass the filters for the different pairings?
length(unique(conv_filt$site[conv_filt$conv_ttPass]))
length(unique(conv_filt$site[conv_filt$conv_pair=='VV' & conv_filt$conv_pass]))
length(unique(conv_filt$site[conv_filt$conv_taPass]))
length(unique(conv_filt$site[conv_filt$conv_pair=='VH' & conv_filt$conv_pass]))
length(unique(conv_filt$site[conv_filt$conv_aaPass]))
length(unique(conv_filt$site[conv_filt$conv_pair=='HH' & conv_filt$conv_pass]))

#how many genes have at least one filter passing position for the different pairings?
length(unique(conv_filt$ortholog[conv_filt$conv_ttPass]))
length(unique(conv_filt$ortholog[conv_filt$conv_pair=='VV' & conv_filt$conv_pass]))
length(unique(conv_filt$ortholog[conv_filt$conv_taPass]))
length(unique(conv_filt$ortholog[conv_filt$conv_pair=='VH' & conv_filt$conv_pass]))
length(unique(conv_filt$ortholog[conv_filt$conv_aaPass]))
length(unique(conv_filt$ortholog[conv_filt$conv_pair=='HH' & conv_filt$conv_pass]))


#remove extra columns
# conv_filt<-conv_filt %>% select('ortholog', 'pos', 'site', 'b1post', 'b2post', 'b1Clade', 'b2Clade', 'conv_pair', 'type', 'genConvergent', 'conv_pass')
...
pctdf %>% 
	...
	ggplot() +
	geom_violin(aes(x=`Phenotype Pair`, y=`Percent Convergent`, fill=`Phenotype Pair`))
...
conv_filt %>%
	...
	ggplot() +
		...
		labs(x='Convergence pair', y='Gene count', title='Convergence and positive selection') +
		coord_flip()
...
#BREAK DOWN BY WHERE THE POSITIVE SELECTION IS
#!!!note, this doesn't seem like the right was to do this, 
#because the VV and HH sets get '2 shots' at being positive,
#whereas the VH convergence sets only gets one shot for the two positve checks
...
pctV <- conv_filt %>%
	filter(conv_pass) %>%
	mutate(posV=
		(!grepl('Anti', b1Clade) & c1PosGene) | (!grepl('Anti', b2Clade) & c2PosGene) ) %>%
	mutate(posH=
		(grepl('Anti', b1Clade) & c1PosGene) | (grepl('Anti', b2Clade) & c2PosGene) ) %>%
	group_by(cp, conv_pair, posV) %>%
	summarize(N=n()) %>%
	filter( posV ) %>%
	mutate(positive='V') %>%
	mutate(total = nt) %>%
...
rbind(pctV, pctH) %>% 
	mutate(group=paste(positive, conv_pair, sep='.')) %>%
	mutate(group=factor(group, levels=c('V.VV', 'V.VH', 'H.VH', 'H.HH'))) %>%
	ggplot() +
		geom_boxplot(aes(x=group, y=pct, fill=conv_pair)) + 
		labs(x='Positive selection and covnergence grouping', y='Perecent tested genes', title="DON'T USE THIS") +
		scale_x_discrete(labels = c(
			bquote(Pos[V]~Conv[VV]),
			bquote(Pos[V]~Conv[VH]),
			bquote(Pos[H]~Conv[VH]),
			bquote(Pos[H]~Conv[HH])))
